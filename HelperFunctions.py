...
def format_concepto(html_storage_txt, dictConcepto):
    ...
    txtDependencias = ", ".join(
        [
            ...
            for x in dictConcepto["listDepend"]
        ]
    )
    ...
    if indexDepen != -1:
        retorno = (
            retorno[0 : (indexDepen + len(patron))]
            + txtDependencias
            + retorno[(indexDepen + len(patron) + 6) : len(retorno)]
        )
    ...
    if indexDepen != -1:
        ...
        retorno = (
            retorno[0 : (indexDepen + len(patron) + 9)]
            + dictConcepto["idConcepto"]
    ...
    if indexDepen != -1:
        try:
            retorno2 = (
                retorno[0 : (indexDepen + len(patron))] + dictConcepto["campoGrata"]
            )
...
def get_page_info(config, auth, pageid):
    url = "{base}/{pageid}".format(base=config["CONFLUENCE"]["BASE_URL"], pageid=pageid)
...
def add_page(config, auth, idConcepto, textoConcepto):
    ...
    print(
        "Se agregó la página '%s' para el concepto %s"
        % (str("Concepto " + idConcepto), str(idConcepto))
    )
